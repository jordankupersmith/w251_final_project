...
# $SPARK_HOME/bin/spark-submit --packages \TargetHolding/pyspark-cassandra:0.3.5 --conf spark.cassandra.connection.host=127.0.0.1/root/queries/mattquery4.py
...
def query_first_then_join(sc):
    ...
    for date_table in tables_to_query:
        ...
        # .persist()
            ...
            # .sortByKey(False)
            # .filter(lambda x: isinstance(x["view_count"], int)) \
