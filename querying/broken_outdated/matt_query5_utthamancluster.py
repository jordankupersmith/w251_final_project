...
def query_first_then_join(sc):
    ...
    rdd_list_count = 0
    schema = StructType([StructField('page_name', StringType()), StructField('view_count', IntegerType())])
    ...
    for date_table in tables_to_query:
        ...
        if rdd_list_count == 0:
            ...
            current_df = sc.createDataFrame(current_monthly_table, schema)
            # .persist()
            # .where("language=?", "en") \
    ...
    combined_rdd = combined_df.rdd#.map(tuple)
